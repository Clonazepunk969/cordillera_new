...
import { createKpi, getKpis } from "../service/kpiService";
import "./analyst.css";
...
export default function AnalystDashboard({ onLogout }) {
    ...
    const [kpiMessageType, setKpiMessageType] = useState("");
    ...
    const handleCreateKpi = async (e) => {
        ...
        try {
            await createKpi({
                name: kpiForm.name,
                type: kpiForm.type,
                inputValue: Number(kpiForm.inputValue),
            });

            setKpiMessage("KPI creado correctamente");
            setKpiMessageType("success");
        ...
        <section className="report-workspace">
            <article className="analyst-card">
                ...
                <p className="section-subtitle">
                    Registra indicadores para alimentar el dashboard en tiempo real.
                </p>

                <form className="analyst-form" onSubmit={handleCreateKpi}>
                    <div>
                    ...
                    <div>
                        ...
                        <select
                            value={kpiForm.type}
                            onChange={(e) =>
                                setKpiForm({ ...kpiForm, type: e.target.value })
                            }
                        >
                    ...
                    <div>
                        ...
                        <input
                            type="number"
                            value={kpiForm.inputValue}
                            onChange={(e) =>
                                setKpiForm({ ...kpiForm, inputValue: e.target.value })
                            }
                            placeholder="Ej: 150"
                            required
                        />
                    ...
                    {kpiMessage && (
                        <span className={`analyst-message ${kpiMessageType}`}>
                            {kpiMessage}
                        </span>
                    )}
                </form>
            ...
            <article className="analyst-card">
                ...
                <p className="section-subtitle">
                    Indicadores disponibles desde el microservicio KPI.
                </p>
            ...
            <section className="analyst-stats">
                ...
                <article className="analyst-stat-card">
                    ...
                    <div>
                        ...
                        <p>Indicadores activos</p>
                        <small>↑ Desde microservicio KPI</small>
                    </div>
                </article>
                ...
                <header className="analyst-topbar">
                    <div className="topbar-left">
                        <div>
                            ...
                            <p>Genera reportes, crea KPIs y analiza información en tiempo real</p>
                        </div>
                    </div>
...
function KpiTable({ kpis }) {
    if (kpis.length === 0) {
        return <div className="empty-analysis">No hay KPIs registrados.</div>;
    }
    ...
    return (
        <div className="analyst-table">
            ...
            {kpis.map((kpi) => (
                <div className="analyst-table-row no-actions" key={kpi.id}>
                    <div className="table-title">
                        <span className="table-icon">◔</span>
                        <strong>{kpi.name}</strong>
                    </div>

                    <span className="type-chip">{kpi.type}</span>
                    <span>{kpi.inputValue}</span>
                    <span>{kpi.resultValue}</span>
